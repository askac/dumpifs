...
struct startup_header {
	unsigned int	signature;			/*I  Header sig, see below*/
	...
	unsigned int	startup_vaddr;		/*I  Virtual Address to transfer*/
										/*   to after IPL is done*/
	unsigned int	paddr_bias;			/*S  Value to add to physical address*/
										/*   to get a value to put into a*/
										/*   pointer and indirected through*/
	unsigned int	image_paddr;		/*IS Physical address of image*/
	unsigned int	ram_paddr;			/*IS Physical address of RAM to copy*/
										/*   image to (startup_size bytes copied)*/
	unsigned int	ram_size;			/*S  Amount of RAM used by the startup*/
										/*   program and executables contained*/
										/*   in the file system*/
	unsigned int	startup_size;		/*I  Size of startup (never compressed)*/
	unsigned int	stored_size;		/*I  Size of entire image*/
	unsigned int	imagefs_paddr;		/*IS Set by IPL to where the imagefs is when startup runs*/
	unsigned int	imagefs_size;		/*S  Size of uncompressed imagefs*/
	unsigned short	preboot_size;		/*I  Size of loaded before header*/
	unsigned short	zero0;				/*   Zeros */
	unsigned int	zero[3];			/*   Zeros */
	unsigned int	info[48];			/*IS Array of startup_info* structures*/
};
